...
from scribo_fashn_ai import ScriboFashnAI, AsyncScriboFashnAI, DefaultAioHttpClient
from scribo_fashn_ai._utils import is_dict
...
logging.getLogger("scribo_fashn_ai").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[ScriboFashnAI]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with ScriboFashnAI(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncScriboFashnAI]:
